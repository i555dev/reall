(() => {
  ...
  function renderFrameWords(step) {
    ...
    wordParticles = words.map((word, index) => createFrameWord(word, index, words.length));
  ...
  function renderSoul(step) {
    ...
    feelingPanel.innerHTML = words.map((word, index) => {
  ...
  function renderStepTabs() {
    const steps = projects[projectIndex].steps;
  ...
  function setCover(nextProjectIndex = projectIndex) {
    ...
    if (experience) {
      experience.style.setProperty('--neural-opacity', '0.34');
  ...
  $('aboutOpen').addEventListener('click', () => openModal(about));
  ...
  window.addEventListener('keydown', (event) => {
    ...
    if (event.key === 'ArrowRight') {
      if (viewMode === 'cover') setState(projectIndex, 0);
      else setState(projectIndex, stepIndex === steps.length - 1 ? 0 : stepIndex + 1);
    }

    if (event.key === 'ArrowLeft') {
      if (viewMode === 'step' && stepIndex === 0) setCover(projectIndex);
      else if (viewMode === 'step') setState(projectIndex, stepIndex === 0 ? steps.length - 1 : stepIndex - 1);
    }
  ...
  function resize() {
    ...
    const count = width < 900 ? 75 : 245;//плотность нейросети
  ...
  function draw() {
    ...
    const max = width < 900 ? 165 : 220; // кол-во связей нейросети
    ...
    for (let i = 0; i < points.length; i++) {
      for (let j = i + 1; j < points.length; j++) {
        ...
        if (dist < max) {
          const alpha = (1 - dist / max) * 0.18; // "громкость" линии
          ctx.shadowBlur = 0;
          ctx.strokeStyle = `rgba(255,255,255,${Math.min(0.14, alpha)})`;
          ctx.lineWidth = 1;
          ctx.beginPath();
          ctx.moveTo(a.x, a.y);
          ctx.lineTo(b.x, b.y);
